...
args = sys.argv[1:]
assert args, "Font size must be supplied as an argument. For example, 'python fontsize.py 1' for a font size of 12."
...
def mutate_lines(filename:str, lines: List[str]) -> List[str]:
    ...
    for index, item in enumerate(lines.copy()):
        if item.rstrip().startswith(gfs):
            new = f"{gfs} = {fontsize}; % {comment}\n"
            lines[index] = new
