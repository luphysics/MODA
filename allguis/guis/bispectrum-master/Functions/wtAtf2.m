...
coib1 = ceil(abs(opt.wp.t1e * fs * (opt.wp.ompeak./(2 * pi * fr))));
coib2 = ceil(abs(opt.wp.t2e * fs * (opt.wp.ompeak./(2 * pi * fr)))); %cone of influence edges
...
out = ((opt.wp.ompeak / (2 * pi * fr))^(1-p)) * ifft(cc, NL); % calculate WT at each time
