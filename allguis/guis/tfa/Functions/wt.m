...
%Newman) so as no longer to filter negative frequencies when preprocessing is off; this involved
...
%frequencies.
%The Morlet wavelet here is defined over the whole frequency
%scale, as in the main body of the 2015 paper rather than the supplementary
%material, and therefore does not need changing. I notice that the default
...
if nargin>2 && isstruct(varargin{1})
    ...
    if isfield(copt,'Wavelet'), cvv=copt.Wavelet; if ~isempty(cvv), Wavelet=cvv; end, end
    ...
    if isfield(copt,'nv'), cvv=copt.nv; if ~isempty(cvv), nv=cvv; end, end
...
for vn=vst:2:nargin-2
    ...
    elseif strcmpi(varargin{vn},'nv'), if ~isempty(varargin{vn+1}), nv=varargin{vn+1}; end
