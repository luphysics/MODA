%Dmytro Iatsenko's codes below have been slightly modified (by Julian
%Newman) so as no longer to filter negative frequencies; this involved
%removing the "set to zero at negative frequencies" command.  Note, however,
%that the filtering prior to padding (if chosen) still includes negative
%frequencies.
%
%Version 1.01 stable
%**************************************************************************
...
%------------------------------Documentation-------------------------------
%
% [WFT,freq,Optional:wopt]=wft(sig,fs,Optional:'PropertyName',PropertyValue)
% - calculate windowed Fourier transform [WFT] of a signal [sig] sampled at [fs] Hz.
% 
% INPUT:
% sig - signal for which to calculate WFT
% fs  - sampling frequency of the signal
% 
% Properties: ({{...}} denotes default)
% ################################ BASIC ##################################
% 'fmin':value (default = 0)
%            minimal frequency for which to calculate WFT
% 'fmax':value (default = fs/2, i.e. the Nyquist frequency)
%            maximal frequency for which to calculate WFT
...
%            wft(signal(n1:n2),fs,'Padding',{signal(1:n1-1),signal(n2+1:end)});
%            the same applies if you can predict signal out of the time-limits
% 'Plot':{{'off'}} - do not plot anything
%          'amp'   - plots WFT amplitude (i.e. its absolute value) together
%                    with line denoting the cone of influence
%          'amp+'  - additionally shows time-averaged WFT amplitude
%          'amp++' - additionally shows 95% range of WFT amplitude
%          'pow','pow+','pow++' - the same but for the WFT power (i.e. its 
%                                 squared modulus)
%           IMPORTANT: to avoid plotting huge data (in which case it might
%           be very slow to render and modify the figure, and MatLab can
%           even crash), the plotted WFT is resampled to have no more than
%           few data points displayed per pixel (for the current screen
%           resolution). Unless 'Display' is 'off', it will always notify if
%           the WFT size exceeds the number of pixels on the plot and the
...
%           structure of the original WFT. If one wants to investigate the
%           resultant plot in fine details (and not only see how it looks),
%           then the original, full WFT might be displayed without resampling
...
% 'CutEdges':{{'off'}}|'on'
%           determine should WFT coefficients be set to NaNs out of the cone
%           of influence (see [1]); set it to 'on' if you wish to analyze
%           only WFT within the cone of influence (which is recommended if
%           you want to estimate e.g. only the time-averaged quantities).
% ############################## ADVANCED #################################
% 'Window':{{'Gaussian'}}|'Hann'|'Blackman'|'Exp'|'Rect'|'Kaiser-a'|{@(xi)fwt(xi),[xi1,xi2],@(t)twf(t),[t1,t2]}
%            window used in WFT calculation, for a list of all supported
...
%            while Hann window with [f0=1] -- as {[],[],@(t)(1+cos(2*pi*t/4.4))/2,[-2.2,2.2]}
% 'Preprocess':{{'on'}}|'off'
...
%            the signal in the band of interest [fmin,fmax], for which WFT
%            is calculated
% 'RelTol':value (default = 0.01) (in [1] commonly referred as \epsilon)
%            relative tolerance (e.g. 0.01 means 1%), which specifies cone
%            of influence for WFT (i.e. range of WFT coefficients which are
...
%
% OUTPUT:
% WFT  - windowed Fourier transform of the signal [sig],
...
% freq - frequencies corresponding to rows of WFT
% wopt - structure with all parameters of the window and simulation
%
%-------------------------------Examples-----------------------------------
%
% [WFT,freq]=wft(sig,100,'fmin',0.1,'fmax',10,'f0',2)
% given a signal [sig] sampled at 100 Hz, returns its WFT [WFT] based on a
...
% [WFT,freq]=wft(sig,fs,opts);
% You can also add further parameters in the usual way, e.g.
% [WFT,freq]=wft(sig,fs,opts,'Display','off');
% If you add a parameter contained in [opts], it will be ovewritten, e.g.
% [WFT,freq]=wft(sig,fs,opts,'fmax',10);
% will change 'fmax' property from 12 specified in [opts.fmax] to 10.
%
% When one needs to calculate WFT using the same window and simulation
% parameters, but for different signals, it is a good idea to do it like
% [WFT1,freq1,wopt]=wft(sig1,fs1,...(parameters));
% [WFT2,freq2]=wft(sig2,fs2,wopt); [WFT3,freq3]=wft(sig3,fs3,wopt); ...
% This will also avoid recalculating window parameters each time, thus
% slightly speeding up the computations.
%
% The same can be done if you calculated WFT of a signal but want to obtain
% it using some different parameter (e.g. Padding), for example
% [WFT1,freq1,wopt]=wft(sig,fs,'Padding',0,...(other parameters));
% [WFT2,freq2]=wft(sig,fs,wopt,'Padding','predictive');
...
function [WFT,freq,varargout] = wft(signal,fs,varargin)
...
if nargin>2 && isstruct(varargin{1})
    ...
    if isfield(copt,'Window'), cvv=copt.Window; if ~isempty(cvv), Window=cvv; end, end
    if isfield(copt,'f0'), cvv=copt.f0; if ~isempty(cvv), f0=cvv; end, end
    if isfield(copt,'fmin'), cvv=copt.fmin; if ~isempty(cvv), fmin=cvv; end, end
    if isfield(copt,'fmax'), cvv=copt.fmax; if ~isempty(cvv), fmax=cvv; end, end
    if isfield(copt,'fstep'), cvv=copt.fstep; if ~isempty(cvv), fstep=cvv; end, end
    if isfield(copt,'Padding'), cvv=copt.Padding; if ~isempty(cvv), PadMode=cvv; end, end
    if isfield(copt,'RelTol'), cvv=copt.RelTol; if ~isempty(cvv), RelTol=cvv; end, end
    if isfield(copt,'Preprocess'), cvv=copt.Preprocess; if ~isempty(cvv), Preprocess=cvv; end, end
    if isfield(copt,'Plot'), cvv=copt.Plot; if ~isempty(cvv), PlotMode=cvv; end, end
    if isfield(copt,'CutEdges'), cvv=copt.CutEdges; if ~isempty(cvv), CutEdges=cvv; end, end
    if isfield(copt,'wp')
        recflag=0;
        for vn=vst:2:nargin-2
            if strcmpi(varargin{vn},'Window') && ~isempty(varargin{vn+1}), recflag=1; end
            if strcmpi(varargin{vn},'f0') && ~isempty(varargin{vn+1}), recflag=1; end
            if strcmpi(varargin{vn},'RelTol') && ~isempty(varargin{vn+1}), recflag=1; end
        end
        wp=copt.wp;
    end
...
for vn=vst:2:nargin-2
    ...
    elseif strcmpi(varargin{vn},'f0'), if ~isempty(varargin{vn+1}), f0=varargin{vn+1}; end
    elseif strcmpi(varargin{vn},'fmin'), if ~isempty(varargin{vn+1}), fmin=varargin{vn+1}; end
    elseif strcmpi(varargin{vn},'fmax'), if ~isempty(varargin{vn+1}), fmax=varargin{vn+1}; end
    elseif strcmpi(varargin{vn},'fstep'), if ~isempty(varargin{vn+1}), fstep=varargin{vn+1}; end
    elseif strcmpi(varargin{vn},'Padding'), if ~isempty(varargin{vn+1}), PadMode=varargin{vn+1}; end
    elseif strcmpi(varargin{vn},'RelTol'), if ~isempty(varargin{vn+1}), RelTol=varargin{vn+1}; end
...
%Produce warning if there are no WFT in the cone-of-influence
if wp.t2e-wp.t1e>L/fs
    if ~strcmpi(DispMode,'off')
        fprintf(2,'--------------------------------------------- Warning! ---------------------------------------------\n');
        wstr='';
        wstr=[wstr,sprintf('For used window function parameters and signal time-length there are no WFT coefficients determined with specified accuracy %1.2e.\n',RelTol)];
...
freq=(ceil(fmin/fstep)*fstep:fstep:floor(fmax/fstep)*fstep)'; % frequencies (determined solely by fstep, which by default depends only on window parameters)
...
if iscell(PadMode)
    ...
    if strcmpi(p1,'predictive') || strcmpi(p2,'predictive')
        if strcmpi(DispMode,'on'), fprintf('Applying predictive padding: '); end
        dflag=1;
    end
    ...
    elseif strcmpi(p1,'predictive')
        if strcmpi(DispMode,'on'), fprintf('\b to the left - '); end
        w=2.^(-(L/fs-(1:L)/fs)/(wp.t2h-wp.t1h));
        padleft=fcast(flipud(signal),fs,n1,[max([fmin,fs/L]),fmax],min([ceil(SN/2)+5,round(L/3)]),w,DispMode); padleft=flipud(padleft);
    ...
    elseif strcmpi(p2,'predictive')
        if strcmpi(DispMode,'on'), fprintf('\b to the right - '); end
        w=2.^(-(L/fs-(1:L)/fs)/(wp.t2h-wp.t1h));
        padright=fcast(signal,fs,n2,[max([fmin,fs/L]),fmax],min([ceil(SN/2)+5,round(L/3)]),w,DispMode);
        if strcmpi(DispMode,'on'), fprintf(';\n'); end
...
elseif strcmpi(PadMode,'predictive')
    ...
    if strcmpi(DispMode,'on'), fprintf('\b to the left - '); end
    padleft=fcast(flipud(signal),fs,n1,[max([fmin,fs/L]),fmax],min([ceil(SN/2)+5,round(L/3)]),w,DispMode); padleft=flipud(padleft);
    if strcmpi(DispMode,'on'), fprintf('; to the right - '); end
    padright=fcast(signal,fs,n2,[max([fmin,fs/L]),fmax],min([ceil(SN/2)+5,round(L/3)]),w,DispMode);
    if strcmpi(DispMode,'on'), fprintf(';\n'); end
    dflag=1; %to detrend one more time
elseif isnumeric(PadMode)
    padleft=PadMode*ones(n1,1); padright=PadMode*ones(n2,1); %padding with predefined values (by default with zeros)
elseif strcmpi(PadMode,'symmetric'),
    padleft=[zeros(n1-L,1);flipud(signal(1:min([n1,L])))]; padright=[flipud(signal(L-min([n2,L])+1:L));zeros(n2-L,1)]; %symmetric padding
elseif strcmpi(PadMode,'periodic'),
    padleft=[zeros(n1-L,1);signal(L-min([n1,L])+1:L)]; padright=[signal(1:min([n2,L]));zeros(n2-L,1)]; %periodic padding
elseif strcmpi(PadMode,'none')
    NL=L; n1=0; n2=0; padleft=[]; padright=[]; %no padding (quite equivalent to periodic padding)
else
...
if strcmpi(Preprocess,'on')
    fx(ff<=fmin | ff>=fmax)=0; % filter signal in a chosen frequency domain
end
%--------------------------------------------------------------------------
...
for sn=1:SN
    ...
    if ~isempty(fwt)
        ...
        tw(jj)=twf(timewf(jj)).*exp(-1i*2*pi*freq(sn)*timewf(jj)); nid=find(isnan(tw) | ~isfinite(tw));
        if ~isempty(nid) %to avoid NaNs due to numerics, e.g. sin(0)/0
...
if strcmpi(CutEdges,'on'), WFT(:,1:coib1)=NaN; WFT(:,1+L-coib2:L)=NaN; end

%Plotting WFT if specified
...
DispMode='off'; if nargin>5 && ~isempty(varargin{3}), DispMode=varargin{3}; end
...
Nq=ceil((L+1)/2); ftfr=[(0:Nq-1),-fliplr(1:L-Nq)]*fs/L; %FT frequencies
orstd=std(Y); %original signal's standard deviation

v=zeros(L,1); ic=v; frq=v; amp=v; phi=v; itn=0;
if strcmpi(DispMode,'on'), pos=2; fprintf('0%%'); end
while itn<MaxOrder
    itn=itn+1;
    aftsig=abs(fft(Y)); [~,imax]=max(aftsig(2:Nq-1)); imax=imax+1;
    
    %========================= Forward search =============================
    %Find two points between which the minimum occurs
    nf=ftfr(imax);
    ...
    if strcmpi(DispMode,'on') && floor(100*itn/MaxOrder)>floor(100*(itn-1)/MaxOrder)
        cstr=num2str(floor(100*itn/MaxOrder)); fprintf([repmat('\b',1,pos),cstr,'%%']); pos=length(cstr)+1;
    end
end
if strcmpi(DispMode,'on'), fprintf([repmat('\b',1,pos),'100%%']); end
